...
const Timer = () => {
  ...
  const modeColors = {
    pomodoro: { bg: 'bg-red-600', text: 'text-red-600', hex: '#DC2626' },
    shortBreak: { bg: 'bg-teal-600', text: 'text-teal-600', hex: '#0D9488' },
    longBreak: { bg: 'bg-indigo-600', text: 'text-indigo-600', hex: '#4F46E5' },
  };
  ...
  return (
    <>
      ...
      <div className={`min-h-screen ${modeColors[mode].bg} p-8`}>
        <nav className="flex justify-between items-center mb-4">
          <h1 className="text-2xl font-bold text-white">Pomodoro Timer</h1>
          <button
            className={`px-4 py-2 bg-white ${modeColors[mode].text} rounded-full`}
        ...
        <div className="text-center mb-8">
          <a href="https://renedeanda.com/?utm_source=pomodoro" target="_blank" rel="noopener noreferrer" className="text-white hover:underline">
            Made with 🧡 + 🤖 by René DeAnda
          </a>
        </div>
        <div className="max-w-md mx-auto bg-white bg-opacity-20 backdrop-blur-md p-8 rounded-lg shadow-lg">
          ...
          <motion.div
            className="text-8xl font-bold text-white text-center mb-8"
            initial={{ scale: 1 }}
          ...
          <div className="flex justify-center mb-8">
            <button
              className="px-8 py-3 bg-white text-red-600 rounded-full text-xl font-semibold"
              onClick={toggleTimer}
            >
          ...
          <div className="bg-white bg-opacity-80 p-4 rounded-lg mb-4">
            ...
            <ul className="space-y-2">
              {tasks.map(task => (
                <li key={task.id} className="flex items-center justify-between">
                  ...
                  <div className="flex items-center">
                    <button onClick={() => updateTaskEstimate(task.id, -1)} className="text-gray-600 px-2">-</button>
                    <span className="text-gray-800 mx-2">{task.estimatedPomodoros} pomodoros</span>
                    <button onClick={() => updateTaskEstimate(task.id, 1)} className="text-gray-600 px-2">+</button>
                    <button onClick={() => removeTask(task.id)} className="text-gray-600 ml-2">×</button>
                  </div>
                </li>
              ))}
            </ul>
            <form onSubmit={addTask} className="mt-4 flex">
              ...
              />
              <button type="submit" className="bg-red-600 text-white px-4 py-2 rounded-r-lg">Add</button>
            </form>
          </div>
        </div>
        <AnimatePresence>
